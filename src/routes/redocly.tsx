...
export default function Redocly() {
  ...
  const { error, data } = useQuery({
    queryKey: ['spec'],
    queryFn: () => fetch('/musixmatch-oas/swagger-config.yaml').then(async res => YAML.parse(await res.text())),
  })
    ...
    <>
      ...
      <RedocStandalone
        specUrl={
          ...
          'https://raw.githubusercontent.com/lunaiz-corp/musixmatch-oas/main/openapi.yaml'
        }
      />
    </>
  )
